...
export default function OnboardingPage() {
    ...
    useEffect(() => {
        ...
        fetch('/api/v1/auth/me', { headers: { 'Authorization': `Bearer ${getAuthToken()}` } }) // Simplified auth header
            .then(res => res.json())
            .then(data => {
    ...
    const handleConnectWhatsapp = async () => {
        setIsLoading(true);
        try {
            const res = await fetch('/api/v1/factory/whatsapp-number', {
                method: 'PATCH',
    ...
    const handleCatalogUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const res = await fetch('/api/v1/catalog/upload', {
                method: 'POST',
                body: formData
            });
            const data = await res.json();
    ...
    const completeOnboarding = async () => {
        setIsLoading(true);
        try {
            const res = await fetch('/api/v1/factory/complete-onboarding', {
                method: 'PATCH',
