...
export default function AdminFactoriesPage() {
    ...
    const fetchFactories = async (token: string) => {
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://16.170.213.68:8000'}/api/v1/admin/factories`, {
                headers: {
    ...
    const toggleFactoryStatus = async (id: string, currentStatus: boolean) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://16.170.213.68:8000'}/api/v1/admin/factories/${id}/status`, {
                method: 'PATCH',
                headers: {
